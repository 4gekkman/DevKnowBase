...
Подоглавление:
  ...
  # -----------------------------
    ...
    • Суть слотов: шаблонизатор для переплетения контента
    • Область видимости при компиляции

      ▪ Шаблон родителя компилируется в его области видимости
      ▪ Шаблон доч.компонента компилируется в его области видимости
      ▪ Не совершай распространённую ошибку, связанную с областями видимости




Распределение контента: переплетение шаблонов родит./дочерн. компонентов
Родит.шаблон компилируется в родит.области видимости, а дочерний в дочерней
...
> Слоты
  ...
  • Простое объяснение принципа работы слотов

    ▪ Есть 2 компонента
      - Родитель и потомок. Компонент parent, компонет child. 
      - Parent вызывает child в своём шаблоне.

    ▪ В шаблоне компонента parent

      

    ▪ В шаблоне компонента child
...
Один компонент
У него шаблон
Вызов через псевдоэл. в шаблоне vue-экзепляра или vue-компонента
Интеграция содержимого псевдоэлемента в шаблон компонента
    ...
    ▪ Есть 2 компонента
      - Родитель и потомок. 
      - Компонент parent, компонет child.
    ...
    ▪ В шаблоне компонента child
      ...
      - Шаблон parent вызывает компонент child.
      - Внутри эл-та child тоже есть пределённый контент.
      - Главный вопрос, как его распихать в шаблоне child?
      - Код:

        ...
        <child>
          <p slot="header">Два</p>      | 
          <p>Body</p>                   | контент внутри child
          <p slot="footer">Четыре</p>   | 
        </child>
        ...
  ...
  В шаблоне компонента comp..

  <slot name="header"></slot>   | куда вставить <el slot="header"> из контента внутри <comp></comp>
  <slot></slot>                 | куда вставить всё остальное из контента внутри <comp></comp>
  ...
  # Ключевые понятие концепции слотов

    • Слоты, как шаблонизатор
    • Шаблон родительского компонента (ШРК)
    • Шаблон дочернего компонента (ШДК)
    • Псевдоэлемент дочернего компонента (ПДК)
    • 


  # О шаблонизации с помощью слотов

    • Компоненты в шаблоне компонента
      - В шаблоне компонента можно использовать компоненты.
      - Вну
...
Псевдоэлемент дочернего компонента
Контент внутри ПДК в родительском шаблоне
Псевдоэлементы slot в шаблоне дочернего компонента
Именованные слоты и атрибут slot в шаблоне родительского компонента
  ...
  # Суть слотов: шаблонизатор для переплетения контента
    ...
    • Слоты, как шаблонизатор
      - Идея слотов пришла из Web Components.
      - Слоты помогают при использовании одних компонентов внутри других.
      - Слоты позволяют обозначить места внутри, где рендерить шаблоны.
      - Принцип работы всего этого, как у шаблонизатора.

    • Компоненты-шаблоны
      - Можно делать компоненты-шаблоны.
      - По аналогии с шаблонами, например, в Laravel.
      - Допустим, вот шаблон документа:

        <div class="container">

          <!-- Здесь слот для компонента-хедера -->
          <!-------------------------------------->
          <header>
            <slot name="header"></slot>
          </header>

          <!-- Здесь слот для компонента-контента -->
          <!---------------------------------------->
          <main>
            <slot></slot>
          </main>

          <!-- Здесь слот для компонента-футера -->
          <!-------------------------------------->
          <footer>
            <slot name="footer"></slot>
          </footer>
