...
////======================////
  ...
  //-----------------------//
    ...
    # Сравнение Vue с другими фрейворками; его обзор на 2 часа.
        https://www.youtube.com/watch?v=7pmw5gvWAf8&t=2716s
...
////=========================////
  ...
  # X.  Практика и наработки
    ...
    X1. Интересные ресурсы из Vue Awesome
...
> Альтернатива наблюдаемым: слежение за св-ми с помощью watch
  ...
  • Пример применения watch
    ...
    - Код:
      ...
      ▪ Решение через наблюдаемую (тоже, что метод, только кэшируется) [ХОРОШО]
...
> Слоты: распределение контента
  ...
  # Слоты: в чём суть кратко
    ...
    • Замечание об областях видимости при компиляции шаблонов
      ...
      - Частая ошибка, обращение к св-ву модели дочернего
...
> Ключевые понятия Vuex: подробности и использование
  ...
  # Геттеры
    ...
    • Обоснование потребности в геттерах
      ...
      - И N компонентам требуется доступ к фильтрованной версии.
  ...
  # Мутации
    ...
    • Мутации обязаны быть синхронными
...
Подоглавление:
  ...
  # Отделение компонентов от роутинга
    ...
    ▪ Управляем вход.парам.компонента в св-ве props объекта-роутера
    ▪ Возможные значения для props в объекте-роутере
  ...
  # Режим HTML5 History
    ▪ Режим по умолчанию для Vue-router - hash
    ▪ Режим history
    ▪ Проблема с 404 в режиме history: путь по умолчанию в Vue-router

  # Навигационные хуки; стражи навигации
    ▪ Пошаговый процесс навигации; какие хуки когда срабатывают
    ▪ Некоторые особенности навигационных хуков
      ▪ Нав.хуки в основном нужны для редиректа/отмены перехода
      ▪ Нав.хуки на 3-х уровнях: глобальный, роут, компонент
      ▪ Enter/Leave-хуки не срабатывают при изменении парам.роутинга/querystring
      ▪ Во всех хуках с next, ты обязан в конце вызвать эту ф-ию
    ▪ Справочник навигационных хуков

      >> Глобальные нав.хуки <<
      ▪ beforeEach        | Перед навигацией, и всеми in-component нав.хуками, и асинх.подтягиванием компонентов роута
      ▪ beforeResolve     | Перед навигацией, НО ПОСЛЕ всех in-component нав.хуков, и асинх.подтягивания компонентов роута
      ▪ afterEach         | Срабатывает после того, как навигация подтверждена (confirmed)

      >> Нав.хуки на уровне роута <<
      ▪ beforeEnter       | Аналог beforeEach, но на уровне роута.

      >> Нав.хуки на уровне компонента <<
      ▪ beforeRouteEnter  | Аналог beforeEach, но на уровне компонента
      ▪ beforeRouteUpdate | Когда роут изменился, но компонент переиспользуется
  ...
  # Vue-router API
    ...
    • Опции конструктора роутера
      ...
      ▪ linkExactActiveClass        | Глобальный active class for exact matches для <router-link>
      ▪ scrollBehavior              | Управление скроллом ф-ией scrollBehavior в роутере
      ▪ parseQuery / stringifyQuery | Парсинг/Стрингификация query string
      ▪ fallback                    | Переключаться ли в режим hash если браузер не поддерживает HTML5 History API (default: true)

    • Экземпляр роутера
      (▪ - свойства, ► - методы)
 
      ▪ app                         | Корневой vue-экземпляр, с которым был связан роутер
      ▪ mode                        | Режим, в котором сейчас работает роутер (hash/history/abstract)
      ▪ currentRoute                | Ссылка на текущий выбранный объект-роут
                                     
      ► beforeEach                  | Глобальный нав.хук. Срабатывает перед навигацией, и всеми in-component нав.хуками, и асинх.подтягиванием компонентов роута.
      ► beforeResolve               | Глобальный нав.хук. Срабатывает перед навигацией, НО ПОСЛЕ всех in-component нав.хуков, и асинх.подтягивания компонентов роута
      ► afterEach                   | Глобальный нав.хук. Срабатывает после того, как навигация подтверждена (confirmed)
      ► push                        | Перейти по новому URI, добавив запись в историю
      ► replace                     | Перейти по новому URI, заменив текущую запись в истории новой
      ► go                          | На сколько шагов перейти по истории назад (n отриц.) или вперёд (n полож.)
      ► back                        | Перейти на 1 запись назад в истории
      ► forward                     | Перейти на 1 запись вперёд в истории
      ► getMatchedComponents        | Массив конструкторов компонентов, связанных с роутом, связанном с выбранным URI
      ► resolve                     | По переданному URI, ищет Location, Route и href
      ► addRoutes                   | Динамически добавить массив новых роутов
      ► onReady                     | Колбэк, кот.будет вызван, когда роутер завершит начальную навигацию
      ► onError                     | Обработчик ошибок навигации

    • Объект-роут

      ▪ Особенности объекта-роута
        ▪ Содержит состояние активного ныне роута
        ▪ Не имеет состояния
        ▪ Способы доступа к объекту-роуту
          ▪ В компоненте через this.$route
          ▪ В колбэках-watch'ерах за $route
          ▪ В ответе на вызов router.match(location)
          ▪ В нав.сторожевых хуках, аргументы to / from
          ▪ В функции scrollBehavior, аргументы to / from

      ▪ Свойства объекта-роута

        ▪ path                      | Полный URI
        ▪ params                    | Объект с динамическими параметрами роутинга, включая *-сегменты.
        ▪ query                     | Объект с параметрами из query string
        ▪ hash                      | Хэш роута (с #), если есть
        ▪ fullPath                  | Полный URL
        ▪ matched                   | Массив сегментов-объектов URI
        ▪ name                      | Имя именованного роута

    • Подмешенные Vue-router'ом в каждый компонент свойства
      (▪ - свойство, • - хук/обработчик)

      ▪ $router                     | Ссылка на экземпляр роутера
      ▪ $route                      | Ссылка на текущий выбранный объект-роут (read-only).

      • beforeRouteEnter            | Аналог beforeEach, но на уровне компонента
      • beforeRouteUpdate           | Когда роут изменился, но компонент переиспользуетс
      • beforeRouteLeave            | Срабатывает перед уходом со старого роута

    • <router-link>

      ▪ Особенности <router-link>
        ▪ Компонент для рендера нав.ссылке в использующем vue-router приложении
        ▪ По умолчанию <router-link> рендерится в DOM-элемент <a></a>
        ▪ Целевое место назначение указывается св-вом to
        ▪ Элемент автом-ки получит CSS-класс "активен", если целевой роут активен
        ▪ Применение CSS-класса "активен" к внешнему элементу
        ▪ Причины, почему <router-link> лучше захардкоженного <a href="...">
          ▪ Работает без изменений в режимах hash/history
          ▪ В HTML5-режиме перехватывает клик, не даёт браузеру сделать F5
          ▪ Использование опции base в режиме HTML5

      ▪ Свойства <router-link>

        ▪ to                        | Значение to будет передано в router.push()
        ▪ replace                   | Вызов router.replace() вместо router.push()
        ▪ append                    | Всегда привязывать относительный path к текущему
        ▪ tag                       | В какой DOM-элемнет должен рендериться <router-link> (<a> по умолчанию)
        ▪ active-class              | Название CSS-класса "активен"
        ▪ exact                     | Отменяет "инклюзивную" стратегию для active-class
        ▪ event                     | Ограничить события, на которые будет реагировать эл-т навигации (click по умолчанию)
        ▪ exact-active-class        | Какой CSS-класс применять для точного (exact) совпадения URI

    • <router-view>

      ▪ Особенности <router-view>
        ▪ Функциональный компонент
        ▪ Если не именованный, отрисовывает связанный с текущим роутом компонент
        ▪ Если именованный, отрисовывает связанные с текущим роутом одноимённые компоненты
        ▪ Отрисованный компонент в шаблоне может иметь вложенные <router-view>
        ▪ Работает с <transition> и <keep-alive>
        ▪ Любые аргументы, кроме name, будут входящими параметрами компонента/ов

      ▪ Свойства <router-view>

        ▪ name                      | Имя для router-view (для отрисовки одноимённого компонента из components).
...
> Отделение компонентов от роутинга
  ...
  • Управляем вход.парам.компонента в св-ве props объекта-роутера
    - Когда мы открываем URI, отрисовывается >= 1 компонента.
    - Через props в объекте-роутере мы можем управлять, какие
      входящие параметры при этом туда будут передаваться.
    - Туда можно передавать любые данные, в том числе и 
      параметры роутинга, отвязывая таким образом компонент
      от них и от роутера.
...
> Режим HTML5 History
  ...
  • Режим по умолчанию для Vue-router - hash
    - Режимом по умолчанию для Vue-router является hash.
    - Он использует хэш URL'а для симуляции полного URL-
      адреса, что позволяет избежать перезагрузки страницы
      при изменении URL.

  • Режим history
    - Можно обойтись и без хэша, используя режим history.
    - Он работает с API history.pushState для достижения той же цели:
    ...
    - При использовании режима history URL выглядит естественно.
    - Например: http://oursite.com/user/id

  • Проблема с 404 в режиме history: путь по умолчанию в Vue-router
    - Чтобы history работал хорошо, надо настроить роутинг на сервере.
    - Там тоже должен быть корневой роут, который вернёт соотв.документ
      независимо от того, какие ещё есть сегменты справа.
    - А когда документ загрузится, в дело вступит клиентский
      роутинг, который покажет нужный/нужные компоненты.
    - При таких раскладах, что бы мы не добавили справа
      от корневого URI, ошибка 404 показана не будет,
      и это многих огорчает.
    - Чтобы с этим справиться, можно указать путь по умолчанию
      в Vue-router:
...
> Навигационные хуки; стражи навигации
  ...
  • Некоторые особенности навигационных хуков
    ...
    ▪ Нав.хуки в основном нужны для редиректа/отмены перехода
      - Так пишут авторы Vue в документации.

    ▪ Нав.хуки на 3-х уровнях: глобальный, роут, компонент
      - Навигационные хуки можно вешать на 3-х разных уровнях.
      - Это добавляет гибкости всей архитектуре.
      - Вот эти 3 уровня:
    ...
    ▪ Enter/Leave-хуки не срабатывают при изменении парам.роутинга/querystring
      - Если надо их отслеживать, используй watch за объектом $route:
    ...
    ▪ Во всех хуках с next, ты обязан в конце вызвать эту ф-ию
      - Например, в хук beforeEach передаются 3 аргумента: to, from, next.
      - Next, это ф-ия, вызов которой декларирует завершения работы хука.
      - Процесс не продолжится, пока next во всех соотв.хуках не будте вызван.
      - Поэтому, её надо всегда вызывать в каждом хуке, иначе навигация будет зависать.
  ...
  • Справочник навигационных хуков
    ...
    ▪ beforeEach
      ...
      ▪ Описание
        - Перед навигацией, и всеми in-component нав.хуками, и асинх.подтягиванием компонентов роута
        - По сути, before-middleware-хук перед актом навигации.
        - Их можно зарегистрировать сколько угодно.
        - Срабатывают в порядке регистрации.
        - Хуки можно регистрировать, используя router.beforeEach:
      ...
      ▪ Могут быть асинхронными
        - И навигация не произойдёт, пока ответы не будут получены.

      ▪ Аргументы, которые получает каждый хук

        ▪ to    | Объект-роут, куда переходим
        ▪ from  | Объект-роут, откуда переходим
        ▪ next  | Ф-ия, вызов которой декларирует завершения работы хука
          - Её надо всегда вызывать в каждом хуке, иначе навигация будет зависать.
          - А результат завершения хука передаётся в аргументе.
          - Вот возможные варианты:

            ▪ next()
              - Перейти к следующему хуку.
              - Если нет, пометить навигацию, как подтверждённую (confirmed).

            ▪ next(false)
              - Отменить текущую навигацию.
              - Восстановить URL браузера из роута from.

            ▪ next('/') or next({ path: '/' })
              - Сделать редирект на указанный URI.
              - Отменяет текущую навигацию, и начинает новую.

            ▪ next(error)
              - В next передаётся экземпляр Error.
              - Отменяет текущую навигацию.
              - Передаёт экземпляр Error в колбэки, зарегистрированные
                с помощью router.onError().

    ▪ beforeResolve
      - Перед навигацией, НО ПОСЛЕ всех in-component нав.хуков, и асинх.подтягивания компонентов роута.
      - Всё остальное по аналогии с router.beforeEach.

    ▪ afterEach
      - Срабатывает после того, как навигация подтверждена (confirmed).
      - В отличие от before-хуков, не получает next 3-им аргументом,
        поэтому и вызывать в них next не надо.
      - Зарегистрировать хук можно так:
    ...
    ▪ beforeRouteEnter
      ...
      ▪ Описание
        - Аналог beforeEach, но на уровне компонента.
        - Всё тоже самое верно, что и для beforeEach.
        
      ▪ Без доступа к this
        - НЕ имеет доступ к this, т.к. вызывается до confirmed.
        - Т.Е. компонент, связанный с новым роутом, ещё не создан.
        
      ▪ Можно задать колбэк для вызова в конце навигации
        - В нём уже будет доступ к this.
        - Колбэк для этого надо передать в next.
        - Пример:
    ...
    ▪ beforeRouteUpdate
      - Когда роут изменился, но компонент переиспользуется.
      - Имеет доступ к this.
      - Вызывается ещё до того, как навигация подтверждена.
      - Например, есть у нас роут с параметрами: /foo/:id
        Пре переходе с /foo/1 на /foo/2, будет использован
        тот же экземпляр компонента.

    ▪ beforeRouteLeave
      - Срабатывает перед уходом со старого роута.
      - Имеет доступ к this.
      - Позволяет предотвратить случайные переходы,
        в стиле "А вы уверены?".
      - Отменить переход, как описано выше, можно
        применив next(false).
...
> Vue-router API
  ...
  • Опции конструктора роутера
    ...
    ▪ linkExactActiveClass
      ...
      ▪ Описание
        - Глобальный active class for exact matches для <router-link>.
    ...
    ▪ scrollBehavior
      ...
      ▪ Описание
        - Управление скроллом ф-ией scrollBehavior в роутере.
        - Работает только в режиме history роутера.
        - Подробнее см.: 
    ...
    ▪ parseQuery / stringifyQuery
      ...
      ▪ Описание
        - Парсинг/Стрингификация query string.
    ...
    ▪ fallback
      ...
      ▪ Описание
        - Переключаться ли в режим hash если браузер не поддерживает HTML5 History API (default: true)
        - Если поставить false, то в ie9 при каждом использовании
          <router-link> вся страница будет обновляться.
        - Это полезно для приложений, которые рендерятся на сервере,
          поскольку режим hash не работает с SSR.
  ...
  • Экземпляр роутера
    ...
    ▪ app
      ...
      ▪ Описание
        - Корневой vue-экземпляр, с которым был связан роутер.
    ...
    ▪ mode
      ...
      ▪ Описание
        - Режим, в котором сейчас работает роутер (hash/history/abstract).
    ...
    ▪ currentRoute
      ...
      ▪ Описание
        - Ссылка на текущий выбранный объект-роут.
    ...
    ► beforeEach
      ...
      ▪ Описание
        - Глобальный нав.хук. Срабатывает перед навигацией, и всеми in-component нав.хуками, и асинх.подтягиванием компонентов роута.
        - Подробнее см.: 
    ...
    ► beforeResolve
      ...
      ▪ Описание
        - Глобальный нав.хук. Срабатывает перед навигацией, НО ПОСЛЕ всех in-component нав.хуков, и асинх.подтягивания компонентов роута.
        - Подробнее см.: 
    ...
    ► afterEach
      ...
      ▪ Описание
        - Глобальный нав.хук. Срабатывает после того, как навигация подтверждена (confirmed).
    ...
    ► back
      ...
      ▪ Описание
        - Перейти на 1 запись назад в истории.
    ...
    ► forward
      ...
      ▪ Описание
        - Перейти на 1 запись вперёд в истории.
    ...
    ► getMatchedComponents
      ...
      ▪ Описание
        - Массив конструкторов компонентов, связанных с роутом, связанном с выбранным URI.
        - Это в основном нужно для server-side реднеринга,
          для осуществления предварительной выборки данных.
    ...
    ► resolve
      ...
      ▪ Описание
        - По переданному URI, ищет Location, Route и href.
        - URI принимает в таком же формате, что <router-link>
        - На выходе выдаёт такой объект:
    ...
    ► addRoutes
      ...
      ▪ Описание
        - Динамически добавить массив новых роутов.
        - Аргументом д.б. объект с роутами-объектами.
    ...
    ► onReady
      ...
      ▪ Описание
        - Колбэк, кот.будет вызван, когда роутер завершит начальную навигацию.
        - Т.Е., когда будут завершены все асинхронные хуки и будут
          готовы асинхронные компоненты, связанные с начальным маршрутом.
        - Полезно при рендеринге на стороне сервера.,
          для обеспечения одинакового результата как на сервере,
          так и на клиенте.
    ...
    ► onError
      ...
      ▪ Описание
        ...
        - Колбэк, будет вызываться при ошибках во время навигации.
        - Для вызова ошибки д.б. использован один из сценариев:
  ...
  • Объект-роут
    ...
    ▪ Особенности объекта-роута
      ...
      ▪ Содержит состояние активного ныне роута
        - Распарсенную информацию из текущего URL.
        - Связанные с текущим URL роуты (route records).

      ▪ Не имеет состояния
        - Любая удачная навигация перезаписывает объект-роут.
    ...
    ▪ Свойства объекта-роута
      ...
      ▪ params
        - Объект с динамическими параметрами роутинга, включая *-сегменты.
        - Будет пустым в случае отсутствия параметров роутинга в URI.
      ...
      ▪ hash
        - Хэш роута (с #), если есть.
        - Если хэш отсутствует, результатом будет пустая строка.
      ...
      ▪ name
        - Имя именованного роута.
        - Если, конечно, сооветствующий роут является именованным.

  • Подмешенные Vue-router'ом в каждый компонент свойства

    ▪ $router
      - Ссылка на экземпляр роутера.

    ▪ $route
      - Ссылка на текущий выбранный объект-роут (read-only).
      - К нему можно прикрепить вотчера (watch).
  ...
  • <router-link>
    ...
    ▪ Особенности <router-link>
      ...
      ▪ Компонент для рендера нав.ссылке в использующем vue-router приложении
        - Допустим, есть у нас vue-приложение.
        - И оно использует для роутинга vue-router.
        - И нам надо в его шаблоне отрисовывать элементы навигации.
        - То есть, при клике по которым vue-router должен осуществлять навигацию.
        - Вот в этих случаях и надо использовать <router-link>.

      ▪ По умолчанию <router-link> рендерится в DOM-элемент <a></a>
        - Причём, с правильным href.
        - То есть его св-во tag по умолчанию равно "a".
        - Однако, если нужно не <a>, можно изменить значение tag.
 
      ▪ Целевое место назначение указывается св-вом to
        - По сути, это декларативный способ навигации.
        - А есть ещё императивный, с помощью push.
        - При клике на <router-link>, push вызывается автоматически.
      ...
      ▪ Элемент автом-ки получит CSS-класс "активен", если целевой роут активен
        - То есть самому не придётся об этом заботиться.
        - Всё уже схвачено до нас.

      ▪ Применение CSS-класса "активен" к внешнему элементу
        - Иногда мы хотим применить его не только к элементу навигации.
        - А ещё к элементу, в который он завёрнут.
        ...
        - В этом случае <a></a> будет ссылкой, с правильным href.
        - Но CSS-класс "активен" получит внешний элемент <li>.

      ▪ Причины, почему <router-link> лучше захардкоженного <a href="...">

        ▪ Работает без изменений в режимах hash/history
          - В отличие от захардкоженного варианта, где
            придётся переписывать ссылки.

        ▪ В HTML5-режиме перехватывает клик, не даёт браузеру сделать F5
          - Событие клика перехватывается.
          - Действие браузера по умолчанию отменяется.
          - Страница не перезагружается.
          - А лишь происходит нав.процесс в рамках vue-router.

        ▪ Использование опции base в режиме HTML5
          - У роутера есть такая опция base.
          - Она содержит базовый URI приложения ("/" по умолчанию).
          - Использование <router-link> позволяет опускать часть base 
            в URI при использовании to.
    ...
    ▪ Свойства <router-link>
      ...
      ▪ to
        ...
        ▪ Описание
          - Значение to будет передано в router.push().
          - Им м.б. строка или объект с описанием места назначеняи.
          - Примеры:
      ...
      ▪ replace
        ...
        ▪ Описание
          - Вызов router.replace() вместо router.push().
          - Например:
      ...
      ▪ append
        ...
        ▪ Описание
          - Всегда привязывать относительный path к текущему.
          - Например, представим, что переходим с /a на /b.
          - Без append мы попадём на /b, а с append на /a/b.
          - Пример:
      ...
      ▪ tag
        ...
        ▪ Описание
          - В какой DOM-элемнет должен рендериться <router-link> (<a> по умолчанию).
      ...
      ▪ exact
        ...
        ▪ Описание
          - Отменяет "инклюзивную" стратегию для active-class.
          - В соотв.с "инклюзивной" стратегией, активный класс,
            например, получит <router-link to="/a">, если 
            текущий путь начинается или равен "/a".
          - Минус в том, что "/", получается, будет активен
            для всех роутов.
          - Вот, чтобы этого не происходило, мы отменяем 
            "инклюзивную" стратегию с помощью exact. Т.Е.
            говорим ему срабатывать только для точных совпадений.
      ...
      ▪ event
        ...
        ▪ Описание
          - Ограничить события, на которые будет реагировать эл-т навигации (click по умолчанию).

      ▪ exact-active-class

        ▪ Тип
          - string
          - "router-link-exact-active" по умолчанию.

        ▪ Описание
          - Какой CSS-класс применять для точного (exact) совпадения URI.
          - Глобально его можно настроить с помощью опции linkExactActiveClass 
            конструктора роутера.
  ...
  • <router-view>
    ...
    ▪ Особенности <router-view>
      ...
      ▪ Функциональный компонент
        - <router-view> является функциональным компонентов.
        - То есть, у него нет состояния.

      ▪ Если не именованный, отрисовывает связанный с текущим роутом компонент
        - С пустым name считается не именованным.
        - Отрисовывает связанный с текущим роутом компонент,
          который находится в св-ве component роута.

      ▪ Если именованный, отрисовывает связанные с текущим роутом одноимённые компоненты
        - Чтобы сделать его именованным, есть св-во name.
        - Отрисовывает связанные с текущим роутом 
          одноимённые компоненты из components.

      ▪ Отрисованный компонент в шаблоне может иметь вложенные <router-view>
        - И эта матрёшка может стремиться к бесконечности.
        - Вложенные компоненты будут использоваться для отрисовки
          с вложенными путями.

      ▪ Работает с <transition> и <keep-alive>
        - Поскольку также является просто компонентом.
        - Например:
      ...
      ▪ Любые аргументы, кроме name, будут входящими параметрами компонента/ов
        - Как видно, <router-view> имеет лишь 1 св-во name.
        - Любые другие атрибуты, переданные <router-view>,
          будут передаваться отрисовываемым компонентам
          в качестве входящих параметров.
        - Хотя, как правило, все эти данные передают через
          props на уровне роутов.
    ...
    ▪ Свойства <router-view>
      ...
      ▪ name
        ...
        ▪ Описание
          - Имя для router-view (для отрисовки одноимённого компонента из components).
...
> Введение
  - Vue Awesome, это репозиторий на github.
  - Это кладезь информации о Vue.
  - В том числе здесь есть ссылки на такие ресурсы
    сообщества, как:
  ...
  - В эту главу я планирую отбирать наиболее интересные из них.
  - Сюда будут попадать только лучшие и проверенные лично мной.
