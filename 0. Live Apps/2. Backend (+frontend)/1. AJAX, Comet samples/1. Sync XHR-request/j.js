 ...
 * Задача:
 *
 * > Сделать демонстрацию работы синхронного XHR-запроса:
 ...
 * 	 	 	 от сервера.
 * 	 	 - Это наглядно продемонстрирует задержку.
 *
 *
 *
 * Архитектура решения:
 ...
 * 		В ней выполнить следующее: [5-10]
 *
 * 5. Кроссбраузерно получить объект-событие с уже примененными IE FIX-ами.
 * 		Также, кроссбраузерно получить target-элемент
 *
 * 6. Записать в span.innerHTML (2) из п.3 текущую дату и время клиента.
 *
 * 7. Выполнить синхронный XHR-запрос методом GET к скрипту p.php.
 *
 * 8. Получить тело ответа, и записать его в innerHTML кнопки (1) из п.3.
 *
 * 9. Записать в span.innerHTML (3) из п.3 текущую дату и время клиента.
 *
 * 10. Записать в span.innerHTML (4) из п.4 разницу между временами из
